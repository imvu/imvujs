...
module({
    ...
    test('single argument', function () {
        ...
        assert.equal('/tests/browser/a.txt', imports.one_a.args[0]);
    });
    ...
    test('nested one', function () {
        ...
        assert.equal('/tests/browser/a.txt', imports.include2.one_a.args[0]);
    });
    test('nested two', function () {
        assert.equal('two', imports.include2.two_c.type);
        assert.equal(1, imports.include2.two_c.args.length);
        assert.equal('/tests/browser/relativePaths/c.txt', imports.include2.two_c.args[0]);
    });
});
